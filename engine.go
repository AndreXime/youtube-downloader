...
func Run(link string, isPlaylist bool, format string, stream chan<- string) error {
	defer close(stream)
	os.MkdirAll(OutputDir, os.ModePerm)

	// Monta argumentos
	args := []string{"--no-warnings"}
	if isPlaylist { args = append(args, "--yes-playlist") } else { args = append(args, "--no-playlist") }
	...
	scanner.Split(func(d []byte, eof bool) (int, []byte, error) {
		if i := bytes.IndexAny(d, "\r\n"); i >= 0 { return i + 1, d[0:i], nil }
		if eof && len(d) > 0 { return len(d), d, nil }
		return 0, nil, nil
	})
